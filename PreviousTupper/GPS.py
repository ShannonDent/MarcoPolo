...
class GPS:
    def __init__(self, uart_number=0, baudrate=9600, tx_pin=0, rx_pin=1):
    ...
    def setup(self):
        self.uart = UART(self.uart_number, baudrate=self.baudrate, tx=Pin(self.tx_pin), rx=Pin(self.rx_pin),rxbuf =1024,timeout = 40, timeout_char = 10)
        self.uart.init(bits=8, parity=None, stop=1, timeout=1000)#change timeout back to 10000
        print("GPS is setup...")

    def read_data(self):
        print('reading...')
        try:
            if self.uart.readline() is not None: #might need to make into try except thingy
                for i in range(10):
