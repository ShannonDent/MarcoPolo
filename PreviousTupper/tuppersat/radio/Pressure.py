...
class Pressure:
    def __init__(self):
        self.sclpin = machine.Pin(17)
        self.sdapin = machine.Pin(16)
    ...
    def read_constants(self):
        cbytes_ = [self.i2c.readfrom_mem(119, poss, 2) for poss in self.registry_positions]
    ...
    def setup(self):
        ...
        #except:
            #print("Pressure Sensor Not connected....")
            #self.data = 0
...
def main():
    pressure = Pressure()
    pressure.setup()
    #cbytes = sensor_controller.read_constants()

    while True:
        pressure.read_data()
        #TEMP = sensor_controller.calculate_temperature(D2, cbytes)
        #print(f'Measured temperature is {TEMP / 100 :2.2f} °C')

        #P = sensor_controller.calculate_pressure(D1, D2, cbytes)
        print(f'Measured pressure is {pressure.data :2.2f} millibars')
